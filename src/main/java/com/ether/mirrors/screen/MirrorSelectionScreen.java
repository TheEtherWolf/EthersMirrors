...
public class MirrorSelectionScreen extends Screen {
    ...
    private static final int SEARCH_H    = 20;  // search box row
    ...
    private int totalPanelH() { return HEADER_H + 2 + SEARCH_H + COLHDR_H + MAX_VISIBLE * ENTRY_H + 10 + 24; }
    private int listTop()     { return panelTop() + HEADER_H + 4 + SEARCH_H + COLHDR_H; }
    ...
    protected void init() {
        ...
        int searchY = pt + HEADER_H + 5;
        searchBox = new EditBox(this.font, pl + 8, searchY, PANEL_W - 16, 14,
                net.minecraft.network.chat.Component.literal("Search"));
        searchBox.setMaxLength(32);
        searchBox.setHint(net.minecraft.network.chat.Component.literal("Search mirrors..."));
    ...
    private void rebuildButtons() {
        // Remove all non-searchBox widgets and re-add them
        clearWidgets();
        if (searchBox != null) addRenderableWidget(searchBox);

        int pl = panelLeft(), pt = panelTop(), lt = listTop();
        int visCount = Math.min(MAX_VISIBLE, filtered.size() - scrollOffset);
        ...
        for (int i = 0; i < visCount; i++) {
            ...
            final java.util.UUID mirrorIdToToggle = info.mirrorId;
            MirrorButton starBtn = MirrorButton.of(starX, ry, STAR_W, BTN_H,
                    Component.literal(starLabel),
                    b -> {
                        MirrorsNetwork.sendToServer(new ServerboundToggleFavoritePacket(mirrorIdToToggle));
                        // Find and update in the full mirrors list, then re-filter
                        for (int j = 0; j < mirrors.size(); j++) {
                            if (mirrors.get(j).mirrorId.equals(mirrorIdToToggle)) {
        ...
        if (scrollOffset > 0) {
            addRenderableWidget(MirrorButton.purple(
                    pl + PANEL_W / 2 - 18, pt + HEADER_H + SEARCH_H - 18, 36, 14,
                    Component.literal("  ^  "), b -> { scrollOffset--; rebuildButtons(); }));
        }
        if (scrollOffset + MAX_VISIBLE < filtered.size()) {
            addRenderableWidget(MirrorButton.purple(
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partial) {
        ...
        int searchY = pt + HEADER_H + 5;
        g.fill(pl + 6, searchY - 2, pr - 6, searchY + 16, UITheme.withAlpha(UITheme.BORDER_MID, 0x88));
        g.fill(pl + 7, searchY - 1, pr - 7, searchY + 15, 0xFF060022);
        ...
        g.drawString(font, "Signal", pl + 295, colHdrY, UITheme.withAlpha(UITheme.TEXT_MUTED, 0xCC), false);
        ...
        if (filtered.isEmpty()) {
            ...
            if (hasAny) {
                // Mirrors exist but none match the search query
                g.drawCenteredString(font, "No mirrors match your search.", pl + PANEL_W / 2, cy - 4, UITheme.TEXT_MUTED);
            } else if ("calling".equals(sourceMirrorType)) {
    ...
    public boolean mouseScrolled(double mx, double my, double delta) {
        ...
        else if (delta < 0 && scrollOffset + MAX_VISIBLE < filtered.size()) { scrollOffset++; rebuildButtons(); }
        return true;
    }
